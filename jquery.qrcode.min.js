 ...
 */
(function(f){var g={size:200,margin:10,type:"text",fitSize:true,render:"canvas",bgColor:"#fff",fgColor:"#000",version:1,errorCorrectLevel:"L"},e="width",a="height",c="px",h=document,b={div:function(u,l,r,d){var s=this,k=s.getModuleCount(),i=u/k,p=u+l*2,m=h.createElement("div");m.style.width=p+c;m.style.height=p+c;m.style.display="inline-block";m.style.position="relative";m.style.backgroundColor=r;for(var t=0;t<k;t++){for(var j=0;j<k;j++){if(!s.isDark(t,j)){continue}var q=j*i+l,o=t*i+l,n=h.createElement("div");n.style.width=i+c;n.style.height=i+c;n.style.position="absolute";n.style.left=q+c;n.style.top=o+c;n.style.backgroundColor=d;m.appendChild(n)}}return m},canvas:function(u,m,q,d){var s=this,l=s.getModuleCount(),i=u/l,j=h.createElement("canvas");if(!(j.getContext&&j.getContext("2d"))){return b.div.call(s,u,m,q,d)}var o=u+m*2,r=j.getContext("2d");j.width=o;j.height=o;r.fillStyle=q;r.fillRect(0,0,o,o);r.fillStyle=d;for(var t=0;t<l;t++){for(var k=0;k<l;k++){if(!s.isDark(t,k)){continue}var p=k*i+m,n=t*i+m;r.fillRect(p,n,i,i)}}return j},svg:function(v,l,s,d){var t=this,k=t.getModuleCount(),i=v/k,q="http://www.w3.org/2000/svg";if(!(h.createElementNS&&h.createElementNS(q,"svg").createSVGRect)){return b.div.call(t,v,l,s,d)}var m=h.createElementNS(q,"svg"),p=v+l*2;m.setAttribute(e,p);m.setAttribute(a,p);m.style.backgroundColor=s;for(var u=0;u<k;u++){for(var j=0;j<k;j++){if(!t.isDark(u,j)){continue}var r=j*i+l,o=u*i+l,n=h.createElementNS(q,"rect");n.setAttribute("x",r);n.setAttribute("y",o);n.setAttribute(e,i);n.setAttribute(a,i);n.setAttribute("fill",d);n.setAttribute("stroke","none");m.appendChild(n)}}return m}};f.fn.qrcode=function(d){if(typeof d=="string"){d={text:d};if(arguments[1]&&typeof arguments[1]=="string"){d.type=arguments[1]}}if(d.type=="url"){d.text="http://"+d.text.replace(/^http:\/\//i,"")}else{if(d.type=="email"){d.text="mailto:"+d.text.replace(/^mailto:/i,"")}else{if(d.type=="tel"){d.text="tel:"+d.text.replace(/^tel:/i,"")}else{if(d.type=="sms"){d.text="smsto:"+d.text.replace(/^smsto:/i,"")}}}}d.text=unescape(encodeURIComponent(d.text));d=f.extend({},g,d);var i=function(){var n=this;var j;try{j=qrcode(d.version,d.errorCorrectLevel);j.addData(d.text);j.make()}catch(m){if(d.version<40){d.version++;i.call(n);return}else{throw m}}if(!b[d.render]){d.render="div"}if(d.fitSize){var k=j.getModuleCount();d.size=Math.round(d.size/k)*k}var l=b[d.render].call(j,d.size,Number(d.margin),d.bgColor,d.fgColor);n.innerHTML="";n.appendChild(l)};return this.each(i)}})(jQuery);
